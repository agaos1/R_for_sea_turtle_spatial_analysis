...
-   Ensure the working directory (folder) is named appropriately (will be used throughout process) and set the working directory, E.g., `dir <- "path/to/directory/`. Here directory is set to the working directory of this documentation R project.
...
packages <- c("tidyverse","mapview","ggspatial","trip","crsuggest","units",
              "plotly","sf", "janitor","lubridate","crsuggest","picMaps")
...
# Packages loading
invisible(lapply(c(packages,"crawlUtils"), library, character.only = TRUE))
```
...
```{r}
locs <- cu_read_wc_dirs(file.path(dir, "raw_data/indiv_tags")) %>% 
  filter(quality!="Z") %>% 
...
```{r, output=FALSE}
meta_data <- read_csv(file.path(dir,"raw_data/Metadata.csv")) %>% 
  clean_names() %>% 
...
(Optional) Here we'll use the `pic_maps` package to get a spatial polygon for the coastline. 
You'll need to download the OSM data the first time you use it. So run the code below if this is the first time using the `picMaps` package. The `set_data_storage()` function will create a directory where the worldwide Open Street Maps (OSM) data will be downloaded. You can specify this directory to be wherever you want that has write permissions. The directory specified below is the default. 
```{r, eval=FALSE}
