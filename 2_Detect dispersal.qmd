...
```{r}
locs$dispersal_plt[[19]] %>% ggplotly() #Change the turtle number as needed
```
...
```{r}
locs$dispersal_plt[[19]] %>% ggplotly()
locs$migr_tbl[[19]][1,]$end <- mdy("8/23/2022")
locs$migr_tbl[[19]][2,]$start <- mdy("8/23/2022")
locs$dispersal_plt[[19]] <- cu_plot_disp(locs$data[[19]], locs$migr_tbl[[19]]) + ggtitle(paste0("ID:",locs$deploy_id[[19]], " / row:",19))
locs$dispersal_plt[[19]]
```
...
```{r}
locs$dispersal_plt[[10]] %>% ggplotly() #Change the turtle number as needed
locs$data[[10]] <- filter(locs$data[[10]], datetime < mdy("10-10-2017"))
locs$dispersal_plt[[10]] <- cu_plot_disp(locs$data[[10]], locs$migr_tbl[[10]]) + ggtitle(paste0("ID:",locs$deploy_id[[10]], " / row:",10))
locs$dispersal_plt[[10]]
```


GAOS: Attempting to re-assign migration for 178577 (row 22) into the pre-migration phase and add the final locations into migration phase. Was able to to first part, but not sure about the second part. Also, there is still a 3rd phase...should I eliminate?
```{r}
locs$dispersal_plt[[22]] %>% ggplotly() #Change the turtle number as needed
locs$migr_tbl[[22]][1,]$end <- mdy("7/10/2022")
locs$migr_tbl[[22]][2,]$start <- mdy("7/11/2022")
locs$migr_tbl[[22]][2,]$end <- mdy("7/14/2022")
locs$dispersal_plt[[22]] <- cu_plot_disp(locs$data[[22]], locs$migr_tbl[[22]]) + ggtitle(paste0("ID:",locs$deploy_id[[22]], " / row:",22))
locs$dispersal_plt[[22]]

locs$migr_tbl[[22]] <- locs$migr_tbl[[22]][1:2,]
locs$dispersal_plt[[22]] <- cu_plot_disp(locs$data[[22]], locs$migr_tbl[[22]]) + ggtitle(paste0("ID:",locs$deploy_id[[22]], " / row:",22))
locs$dispersal_plt[[22]]
```

GAOS: Reassigning some locations to pre-migration phase. I did this and it actually eliminated some locations...because there is a day different in end and start. will redo
```{r}
locs$dispersal_plt[[18]] %>% ggplotly() #Change the turtle number as needed
locs$migr_tbl[[18]][1,]$end <- mdy("7/22/2019")
locs$migr_tbl[[18]][2,]$start <- mdy("7/23/2019")
locs$dispersal_plt[[18]] <- cu_plot_disp(locs$data[[18]], locs$migr_tbl[[18]]) + ggtitle(paste0("ID:",locs$deploy_id[[18]], " / row:",18))
locs$dispersal_plt[[18]]
```

GAOS: Want to incorporate all points into a foraging phase for 176769 (14). I did this, but should we eliminate phase 0 completely? 
```{r}
locs$dispersal_plt[[14]] %>% ggplotly() #Change the turtle number as needed
locs$migr_tbl[[14]][1,]$end <- mdy("6/4/2019")
locs$migr_tbl[[14]][2,]$start <- mdy("6/5/2019")
...
locs$migr_tbl[[14]] <- locs$migr_tbl[[14]][2,]
```
...
```{r}
save(locs, land, vis_graph, file="processed_data/migr_det_data.RData")
```
